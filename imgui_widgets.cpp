...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    if (g.Style.RetroMode)
    {
        ...
        window->DrawList->AddLine(cross_center + ImVec2(+cross_extent + 1.0f, +cross_extent), cross_center + ImVec2(1.0f, 0), cross_col, cross_thickness);
        window->DrawList->AddLine(cross_center + ImVec2(+cross_extent + 1.0f, -cross_extent), cross_center + ImVec2(1.0f, 0), cross_col, cross_thickness);
        window->DrawList->AddLine(cross_center - ImVec2(1.0f, 0), cross_center + ImVec2(-cross_extent - 1.0f, -cross_extent), cross_col, cross_thickness);
        window->DrawList->AddLine(cross_center - ImVec2(1.0f, 0), cross_center + ImVec2(-cross_extent - 1.0f, +cross_extent), cross_col, cross_thickness);
    }
    else
    {
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node)
{
    ...
    if (g.Style.RetroMode)
    {
        ...
        window->DrawList->AddLine(center + ImVec2(-4, -1), center + ImVec2(4, -1), text_col, 1.0f);
        window->DrawList->AddLine(center + ImVec2(-4, 0), center + ImVec2(4, 0), text_col, 1.0f);
    }
    else
    {
        if (hovered || held)
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (is_visible)
    {
        const bool highlighted = hovered || (flags & ImGuiSelectableFlags_Highlight) || g.Style.RetroMode;
